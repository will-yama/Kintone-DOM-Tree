(function () {
	...
	kintone.events.on('app.record.index.show', function (eventobject) {
	    ...
	    //Change the value of the variable below to match the field code you want to extract data from
	    //In the below case, the field with the fieldcode "companyname" is being extracted
		...
		//Extract field data from event object and push into an array
		var array_of_field_data = [];
		for (var k=0; k<eventobject.records.length; k++){
		array_of_field_data.push(eventobject.records[k][field_to_display].value);
		}

		var width = 500,
			height = eventobject.records.length * 25,
			quantity = eventobject.records.length * 2.5,
			types = [ 'select', 'meter', 'button', 'radio', 'checkbox' ],
            kintone_field_data = array_of_field_data;

		var tree = kintone.app.getHeaderSpaceElement(),
			treeRotation = 0;

		tree.style.width = width + 'px';
		tree.style.height = height + 100 + 'px';
		...
		for( var i = 0; i < quantity; i++ ) {
			var element = null,
				...
				greeting = kintone_field_data[ Math.floor( Math.random() * kintone_field_data.length ) ];
			...
			switch( type ) {
				case 'button':
					...
					element.addEventListener ("click", function() {
						alert("o(●´ω｀●)o Merry Christmas!!! o(●´ω｀●)o");
					});
					break;
				case 'select':
					element = document.createElement( 'select' );
					element.setAttribute( 'selected', greeting );
					element.innerHTML = '<option>' + kintone_field_data.join( '</option><option>' ) + '</option>';
					element.style.width = elemenWidth + 'px';
					element.style.height = elemenHeight + 'px';
					break;
				case 'meter':
